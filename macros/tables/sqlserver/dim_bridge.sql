...
{%- macro sqlserver__dim_bridge(src_pk, bridge_fks, source_model, src_ldts, weighting_factor, src_eff, src_exp, src_extra_columns) -%}
...
bridge_current AS (
    ...
    WHERE f.{{ src_exp }} = {{ automate_dv.cast_date(max_datetime, as_string=true, datetime=true) }}
        OR f.{{ src_exp }} > CONVERT(DATE, GETDATE())
),
