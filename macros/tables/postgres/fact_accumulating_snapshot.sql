...
{%- macro postgres__fact_accumulating_snapshot(src_pk, src_fk, source_model, dimensions, milestones, lag_facts, satellites, timespan, src_extra_columns) -%}
...
milestone__{{ mkey }} AS (
    SELECT DISTINCT ON ({% for col in m_pk_expand %}m.{{ col }}{% if not loop.last %}, {% endif %}{% endfor %})
    ...
    ORDER BY {% for col in m_pk_expand %}m.{{ col }}{% if not loop.last %}, {% endif %}{% endfor %}, m.{{ m_date }} DESC
),
...
{%- do sel_ns.parts.append("(fe." ~ lcfg['to'] ~ ")::date - (fe." ~ lcfg['from'] ~ ")::date AS " ~ lname) %}
{%- endfor %}
{%- endif %}
{%- if satellites is mapping %}
{%- for sat_name, sat_config in satellites.items() %}
...
closed_prior AS (
    SELECT
        ...
        {%- endif %}
        t.EFFECTIVE_FROM,
        nv.EFFECTIVE_FROM - INTERVAL '1 microsecond' AS EFFECTIVE_TO,
        FALSE AS IS_CURRENT
    FROM {{ this }} t
