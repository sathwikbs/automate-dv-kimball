...
{%- macro postgres__fact_periodic_snapshot(src_pk, src_fk, src_ldts, source_model, dimensions, satellites, snapshot_period, src_extra_columns) -%}
...
{{ sat_name | lower }}_periodic AS (
    ...
    ORDER BY
        ...
        {%- endfor %}
        date_trunc('{{ sp_lower }}', {{ automate_dv.prefix([sat_ldts], 'sat') }}),
        {{ automate_dv.prefix([sat_ldts], 'sat') }} DESC
),
