...
{%- macro dim_bridge(src_pk, bridge_fks, source_model, src_ldts, weighting_factor=none, src_eff=none, src_exp=none, src_extra_columns=none) -%}

    {{- automate_dv.check_required_parameters(src_pk=src_pk, bridge_fks=bridge_fks,
                                              src_ldts=src_ldts,
                                              source_model=source_model) -}}

    {{- automate_dv.prepend_generated_by() }}

    {{ adapter.dispatch('dim_bridge', 'automate_dv_kimball')(src_pk=src_pk, bridge_fks=bridge_fks,
                                                      source_model=source_model,
                                                      src_ldts=src_ldts,
...
{%- macro default__dim_bridge(src_pk, bridge_fks, source_model, src_ldts, weighting_factor, src_eff, src_exp, src_extra_columns) -%}
...
bridge_latest AS (
    ...
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY {{ automate_dv.prefix(pk_list, 'f') }}
        ORDER BY {{ automate_dv.prefix([src_ldts], 'f') }} DESC
    ) = 1
),
