...
{%- macro dim(src_pk, src_nk, src_ldts, source_model, satellites, scd_type, src_extra_columns, integer_surrogate) -%}

    {%- set scd_type = scd_type | default(1) -%}
    {%- set integer_surrogate = integer_surrogate | default(false) -%}
    ...
    {{ adapter.dispatch('dim', 'automate_dv_kimball')(src_pk=src_pk, src_nk=src_nk,
                                              src_ldts=src_ldts,
                                              source_model=source_model,
...
{%- macro default__dim(src_pk, src_nk, src_ldts, source_model, satellites, scd_type, src_extra_columns, integer_surrogate) -%}
...
sat_timeline AS (
    ...
    {%- set first_pk = first_config['pk'] %}
    {%- set first_ldts = first_config['ldts'] %}
    SELECT
        sat.{{ first_pk }} AS {{ src_pk }},
        sat.{{ first_ldts }} AS EFFECTIVE_FROM
...
{{ sat_name | lower }}_as_of AS (
    ...
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY v.{{ src_pk }}, v.EFFECTIVE_FROM
        ORDER BY {{ sat_name | lower }}_ranked.{{ sat_ldts }} DESC
    ) = 1
),
